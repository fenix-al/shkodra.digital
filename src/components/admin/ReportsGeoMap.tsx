...
import { Crosshair, ExternalLink, Layers3, MapPin, Navigation } from 'lucide-react'
...
const LeafletMap = dynamic(() => import('./ReportsGeoMapCanvas'), {
  ...
  loading: () => (
    <div className="flex h-[560px] w-full items-center justify-center rounded-[28px] border border-white/10 bg-[#06101c] text-sm text-slate-400">
      Duke ngarkuar hartën interaktive...
    </div>
  ),
})
...
export default function ReportsGeoMap({ reports }: Props) {
  ...
  }, [categoryFilter, geolocatedReports, statusFilter])
    ...
    return (
      <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur-md">
        ...
        <p className="mt-4 text-sm text-slate-400">
          Sapo qytetarët të dërgojnë raporte me vendndodhje, këtu do të shfaqet harta interaktive me zoom, tiles reale dhe clustering.
        </p>
      </div>
    )
  }
    ...
    <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur-md">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="flex flex-wrap gap-3 text-xs text-slate-500">
          ...
          <span>Në hartë: <span className="font-semibold text-slate-300">{filteredReports.length}</span></span>
      ...
      <div className="mt-5 rounded-[28px] border border-white/10 bg-black/20 p-4">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Filtrat e Hartës</p>
            <h3 className="mt-1 text-sm font-semibold text-slate-200">Përditësojnë pikat live pa prekur tabelën</h3>
          </div>

          <div className="grid grid-cols-1 gap-3 md:grid-cols-3 xl:w-[720px]">
            <select
              value={statusFilter}
      ...
      {filteredReports.length === 0 ? (
        <div className="mt-5 rounded-[28px] border border-white/10 bg-[#06101c] p-10 text-center text-slate-400">
          Nuk ka raporte me koordinata për filtrat aktualë.
        </div>
      ) : (
        <div className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-[1.2fr_0.8fr]">
          <div className="overflow-hidden rounded-[28px] border border-white/10 bg-[#06101c]">
            <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Hartë e Plotë Interaktive</p>
            ...
            <LeafletMap reports={filteredReports} selectedId={selectedReport?.id ?? null} onSelect={setSelectedId} />
          </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-4">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <button
                    type="button"
                    onClick={() => setSelectedId(selectedReport.id)}
