        ...
        <div className="flex flex-col gap-2">
          ...
          {preview ? (
            <div className="relative rounded-2xl overflow-hidden border border-white/10 aspect-video">
              ...
              <button type="button" onClick={clearPhoto} aria-label="Hiq foton" className="absolute top-2 right-2 p-1.5 rounded-xl bg-black/60 backdrop-blur-sm text-white hover:bg-black/80 transition-all active:scale-95">
                <X size={14} />
              </button>
            </div>
          ) : (
        ...
        <div className="flex flex-col gap-2">
          ...
          {geoStatus === 'ok' && coords ? (
            <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/20">
              ...
              <button type="button" onClick={clearLocation} aria-label="Hiq vendndodhjen" className="p-1 rounded-lg text-slate-500 hover:text-slate-300 transition-colors active:scale-95">
                <X size={12} />
              </button>
            </div>
          ) : geoStatus === 'denied' ? (
            ...
            <button type="button" onClick={requestLocation} disabled={geoStatus === 'loading'} className="flex items-center justify-center gap-2 py-3 rounded-2xl border border-white/5 bg-white/[0.03] text-sm font-semibold text-slate-400 hover:bg-white/[0.06] hover:text-slate-200 transition-all active:scale-95 disabled:opacity-50">
              {geoStatus === 'loading'
                ...
                : <><MapPin size={15} /> Shto vendndodhjen</>
              }
            </button>
          )}
        </div>
